...
func getCustomer(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query(sql, CustomerID)
...
func updateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PUT" {
		...
		_, err = stmt.Exec(CompanyName, ContactName, ContactTitle, Address, City, Region, PostalCode, Country, Phone, Fax, CustomerID)
		if err != nil {
			fmt.Fprintf(w, "Data not found or Request error")
		} else {

			fmt.Fprintf(w, "Customer with CustomerID = %s was updated", CustomerID)
...
func main() {
	...
	r.HandleFunc("/getcustomers", getCustomer).Methods("GET")
